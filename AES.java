...
public class AES{
    ...
    private static int[] SBox = {9,4,10,11,13,1,8,5,6,2,0,3,12,14,15,7}; // S Box defined
	private static int[] SInverseBox = new int[16];
	private static int[] encryptionKey = new int[6]; 
	private static int[] key = new int[3];
	private static int[][] mixColumnMatrix = {{1,4},{4,1}};				// Mix Column is defined
	private static int[][] mixInverseColumnMatrix = {{9,2},{2,9}};		// Inverse of mix column is defined
	...
	private static int merge(int a,int b){		 // merge two 4 bit value into a 8 bit 
		return (int)((a<<4)+b);
	}
	
	private static int multiply(int l , int r){		// Multiply two number according to GF(2^4) 
		int ans=0,finalAns=0;						// x^4+x+1
		ans=r;										// mod value is 0011 = 3
		for(int i=0;i<4;i++){
			if(ans>15)
			ans = (int)((ans%(1<<4))^mod);
			if((l&(1<<i))>0){
	...
	private static int[] mixColums(int[] word,int[][] Matrix){	// Mix Columns operation done on word
	...
	private static int SubNib(int word){				// Substitute according to Sbox
		return (int)((SBox[(word/(1<<4))]<<4)+SBox[(word%(1<<4))]);
	}
	
	private static int SubInverseNib(int word){			// Substitute according to SInverseBox	
		return (int)((SInverseBox[(word/(1<<4))]<<4)+SInverseBox[(word%(1<<4))]);
	}
	
	private static int SubNibRotNib(int word){			// Substitute firstly by swaping the two hexa decimals
		return (int)((SBox[(word%(1<<4))]<<4)+SBox[(word/(1<<4))]);
	}
	
	private static int[] shiftRow(int[] a){				// Shifting Row Operation
		int[] word = new int[2];
	...
	private static void keyGeneration(int[] a){			// Key Generation
	...
	public static void show(int[] a,String s){			// show the binary convertiong of the given integer array
		System.out.println(s + " : ");
		String st="";
		for(int i : a){
			st = Integer.toBinaryString(i);
			if(st.length()<8){
				for(int j=0;j<8-st.length();j++)
					System.out.print('0');
			}
			//System.out.print(st);
		}
	...
	public static String encrypt(String plaintext, String keyText){		// Encryption operation 
		...
		//show(encryptedWord, "Cypher Text : ");
		
		//System.out.println("-------------------------------------------------------------");
		return getBufHexStr(encryptedWord);
    }
	
	public static String decrypt(int[] word,String keyText ){		// decryption function Inverse of encrypt function
		//show(word);
		int[] temp = new int[2];
		int[] key = new int[4];
		for(int i=0;i<2;i++){
